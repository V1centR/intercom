...
class CartController extends AbstractActionController{
 ...
 $test = new Produto();
 $test->getNome();
  //$this->cart = new Container('carrinho');
  //$this->cart->getManager()->destroy();
    ...
    public function addAction() {
       ...
       if(!empty($postdata)){           
        ...
        $date = date('Y-m-d H:i:s');
            ...
            if($query_carrinho->rowCount() == 0){                
            
                #executa strings passadas como parametros na funcao##########################
                $this->object = $this->getServiceLocator()->get('doctrine.entitymanager.orm_default');
                $this->str = "INSERT INTO `$db`.Carrinho SET visitanteId = '$this->idUser' , datahora = '$date'";
                $query = $this->object->getConnection()->prepare($this->str);
                       
                
                if($query->execute()){
                    
                    $this->id_carrinho = $this->object->getConnection()->lastInsertId();

                    #executa insert de itens no carrinho ##########################
                    $this->object = $this->getServiceLocator()->get('doctrine.entitymanager.orm_default');
                    $this->str = "INSERT INTO `$db`.CarrinhoItens SET carrinhoId = '$this->id_carrinho' , produtoId = '$prod_id', quantidade = 1";
                    $query = $this->object->getConnection()->prepare($this->str);
                    $query->execute(); 
                }
                ...
                $this->str_check_prodcart = "SELECT * FROM `$db`.CarrinhoItens WHERE carrinhoid = '$cartId' AND produtoId = '$prod_id' ";
                $query0 = $this->object->getConnection()->prepare($this->str_check_prodcart);
                $query0->execute();
                ...
                if( $query0->rowCount() == 0){
                    ...
                    #executa insert de itens no carrinho ##########################                   
                    $this->str = "INSERT INTO `$db`.CarrinhoItens SET carrinhoId = '$cartId' , produtoId = '$prod_id', quantidade = 1";
                    $query = $this->object->getConnection()->prepare($this->str);
                    $query->execute();   
